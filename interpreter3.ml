...
type command =
  ...
  (* part 3 *)
  | Func of ( const * const * command list)
  | Call
  | Throw
...
and func () =
   (keyword "DefFun") >>((const)<< ws ) +++((const)<< ws)+++ many' command << (keyword "End") >|=
  (fun ((a,b),cmds) -> Func (a,b,cmds) )
...
(* language value *)
(*env = (string*const) list*)
type value =
  | I_val of int
  | B_val of bool
  | S_val of string
  | N_val of string
  | F_val of ((string*value) list* string * string * command list)
  | U_val
...
let rec run (p : prog) (st : stack) (e : env) (log : string list) :
  ...
  (*env = (string*const) list*)
  | Func (N fname, N vname , cmds) :: rest -> run rest st (put e fname (F_val (e,fname, vname,cmds))) log
  
  | Call :: rest ->
    (match st with 
    | x :: F_val y :: st -> (match y with 
                                (env, fname, c, cmds) -> match (run cmds st (put (put env fname (F_val (env,fname, c,cmds)) ) c x) log) with
                                                      | (log, Ok (v :: _)) -> run rest (v :: st) e log
                                                      | (log, Ok _) -> log, StackError
                                                      | error -> error        ) 
    | _ :: _ :: st -> log, TypeError
  ...
  | TryCatch (cmds1, cmds2) :: rest ->
    (match run cmds1 [] e log with
    (*try block succeeds *)
    | (log, Ok (v :: _)) -> run rest (v :: st) e log  
    (*try block fails *)
    | (log, error) -> run (cmds2@rest) ((I_val (to_int_result error)) :: st) e log
     | error -> error)
...
let deb prog =
 let (revlog, stack) = run prog [] [] [] in (List.rev revlog, stack)
